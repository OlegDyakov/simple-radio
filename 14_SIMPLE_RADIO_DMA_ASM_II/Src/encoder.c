...
#define FREQ_MAX  1650 // Максимальное значение частоты KHz
...
/* Подключение выхода энкодера A (PA6) и B (PA7) к таймеру TIM3 */
 ...
 /*  Настройка таймера TIM3 на работу с энкодером */
	 ...
	 // Установка приоритета  2 для прерывания 29.
	 NVIC_IPR7 = 0x00002000;
 ...
 /*  Настройка  кнопки BUT энкодера */
	 ...
	 // Установка приоритета  2 для прерывания 23.
	 NVIC_IPR5 = 0x20000000;
	...
	if(TIM3_SR & TIM_SR_TIF){
		...
		if((c & 1) == 0) {
			...
			if(f != frequency){		  //Защита от дублирования значений частоты
				f = frequency;
				KHZ_to_R(); //Расчет значения R для фильтра
				FreqPrint();
			}
